...
def getALL(input_folder):
	# initialize a dictionary lsit to store the target files
...
def isValid(input_folder, pattern_index: int = 1) -> bool:
	# 1. 检查Pixyz是否安装
	# 2. check file/dir path is valid
	# support extensions right now.
	# @supported_extensions: filter list
		...
		for mdoel_group in ks:
			...
			if f.get(mdoel_group):
				...
				for _model in target_models_to_import:
					if os.path.splitext(_model)[1] not in supported_extensions:
...
def getInfoFromFile(file_path, pattern_index: int):
	...
	return rvt_id, rvt_name, rvt_code, extension
